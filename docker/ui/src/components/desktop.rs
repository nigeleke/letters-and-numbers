...
pub fn desktop() -> Html {
  ...
  let on_solution_change = Callback::from(move |updated_solution: Solution| {
    solution_state.set(updated_solution.clone());
  });
  ...
  let updated_solve_action = SolveAction::using(&goal, &numbers, &solution, on_solution_change.clone());
  ...
  let updated_reset_action = ResetAction::using(&solution,
                                                on_solution_change,
